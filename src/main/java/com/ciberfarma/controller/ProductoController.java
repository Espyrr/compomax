...
public class ProductoController {
	...
	public String listarProductos(@RequestParam(name = "buscar", required = false) String buscar,
			@RequestParam(name = "pagina", defaultValue = "1") int pagina, Model model, HttpSession session) {
	...
	public String eliminarProducto(@PathVariable("id") String id, RedirectAttributes redirectAttrs,
			HttpSession session) {
		Usuario usuario = (Usuario) session.getAttribute("usuarioLogueado");
		if (usuario == null || usuario.getIdTipo().getIdTipo() != 1) {
			return "redirect:/acceso-denegado";
